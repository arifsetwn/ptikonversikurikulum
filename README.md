...
![TailwindCSS](https://img.shields.io/badge/TailwindCSS-4.x-06B6D4?style=flat-square&logo=tailwindcss&logoColor=white)
![PDF.js](https://img.shields.io/badge/PDF.js-6.x-FF6B6B?style=flat-square&logo=adobeacrobatreader&logoColor=white)
...
- **Identitas Visual UMS**: Desain bertema resmi UMS Navy (`#2F3185`), Gold (`#FFB800`), dan Tosca (`#1CBDB3`) dengan animasi micro-interaction.
...
- **Peta Alur Kurikulum & Spring Physics Image Zoom**: Visualisasi alur perkuliahan Semester 1 s/d 8 yang interaktif dengan komponen *Animate UI ImageZoom* (efek zoom halus berbasis fisika pegas layar penuh).
- **6 Learning Path Karir**: Pilihan jalur spesialisasi (AI & Data Science, Software Engineering, Cybersecurity/Cloud, EdTech & E-Learning, Immersive Tech/Game, & Technopreneurship) dengan rekomendasi MK pilihannya.
...
- **Catatan Perubahan Umum & Khusus**: Tab rincian 18 Perubahan Nama MK (termasuk *Literasi Digital* → *Transformasi Digital*), 7 Perubahan Bobot SKS, Mata Kuliah Baru, dan MK Pilihan jadi Wajib.

### 2. Tool Simulasi Konversi Nilai Mandiri (2022 → 2026)
- **Ekstraksi PDF Transkrip Otomatis (Utama)**: Unggah dokumen PDF transkrip Laporan Perkembangan Studi dari **MyAkademik UMS** (`myakademik.ums.ac.id`) untuk ekstraksi otomatis Nama, NIM, daftar seluruh Mata Kuliah 2022, SKS, dan Nilai Huruf.
- **Input Transkrip Excel/CSV & Manual**: Mendukung impor file Excel/CSV (`.xlsx`, `.xls`, `.csv`) serta pencarian mata kuliah manual berbasis Autocomplete dengan navigasi keyboard (`↑`, `↓`, `Enter`).
...
- **Cetak & Ekspor PDF**: Menghasilkan dokumen resmi simulasi mandiri bertanda Kop Prodi PTI UMS yang siap dicetak/diunduh secara langsung.
- **Aksesibilitas & UI/UX Standards**: Memenuhi kriteria accessibility (ARIA labels, minimum 44px touch targets, dan kontras warna tinggi).
...
- **Frontend Framework**: React 19 (`react` ^19.0.0) + TypeScript (`typescript` ^5.7.3)
- **Build Tool**: Vite 6 (`vite` ^6.1.0) dengan konfigurasi relative base path (`./`)
- **Styling & UI**: TailwindCSS v4 (`@tailwindcss/vite` ^4.0.7) + UMS Color System Tokens
- **Animations & Primitives**: Motion / Framer Motion (`motion` ^12.42.2) + Radix UI Slot & Animate UI Primitives
- **PDF Parser (Import)**: PDF.js (`pdfjs-dist` ^6.1.200) untuk parsing PDF Laporan Perkembangan UMS
...
├── src/
│   ├── components/         # Komponen UI (Navbar, Hero, LandingPage, Forms, ResultTable, PDF Modal, dll)
│   │   └── ui/             # Komponen Primitive UI (image-zoom.tsx, background-paths.tsx, button.tsx)
│   ├── data/               # Master Data JSON (mappingData, courses2022, courses2026, landingData)
│   ├── img/                # Asset Gambar (logo_pti.png, alur_kurikulum.png)
│   ├── lib/                # Utility UI (utils.ts dengan clsx & tailwind-merge)
│   ├── types/              # Type Definitions TypeScript (Course, Mapping, Stats, Grade)
│   ├── utils/              # Conversion Engine, PDF Parser (PDF.js), Excel Parser, PDF Generator, & Sample Data
│   ├── App.tsx             # Main App Router & View Controller
│   ├── index.css           # Global Styles & TailwindCSS v4 Directives
│   └── main.tsx            # Application Entrypoint
├── public/                 # Static Assets
├── index.html              # HTML Document Shell
├── package.json            # Dependencies & Scripts (React 19, Tailwind v4, Vite 6)
...
Dengan konfigurasi `base: './'` pada `vite.config.ts`, aplikasi dapat dipasang baik di **root domain** (misal `https://domainanda.com/`) maupun di dalam **subfolder** (misal `https://domainanda.com/kurikulum26/`).
...
4. Ekstrak file ZIP di folder tujuan. Pastikan file `index.html`, folder `assets/`, dan file `.htaccess` berada langsung di dalam folder tersebut:
   ```
