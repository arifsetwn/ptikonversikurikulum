    ...
    <div className="bg-white rounded-3xl border border-slate-200/90 shadow-sm p-6 sm:p-8 mb-8 card-hover-glow">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 py-6">
        ...
        <div className="p-4 rounded-2xl bg-blue-50/90 border border-blue-200 shadow-sm flex flex-col justify-between">
          ...
          <div className="flex items-baseline gap-1.5 my-1">
            ...
            <span className="text-xs text-blue-800 font-bold">/ 4.00</span>
          </div>
        ...
        <div className="p-4 rounded-2xl bg-indigo-50/90 border border-indigo-200 shadow-sm flex flex-col justify-between">
          ...
          <div className="flex items-baseline gap-1.5 my-1">
            ...
            <span className="text-xs text-indigo-800 font-bold">/ 4.00</span>
          </div>
          <div className="flex items-center gap-1 text-[10px] font-bold">
            {ipkDelta > 0 ? (
              ...
              <span className="text-rose-800 font-black flex items-center gap-0.5">
        ...
        <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 flex flex-col justify-between">
          ...
          <div className="flex items-baseline gap-1.5 my-1">
            ...
            <span className="text-xs text-slate-600 font-bold">SKS</span>
          </div>
        ...
        <div className="p-4 rounded-2xl bg-emerald-50/80 border border-emerald-200 flex flex-col justify-between">
          ...
          <div className="flex items-baseline gap-1.5 my-1">
            <span className="text-2xl font-black text-emerald-900">{stats.totalConvertedSks}</span>
            <span className="text-xs text-emerald-800 font-bold">SKS</span>
          </div>
        ...
        <div className="p-4 rounded-2xl bg-amber-50/80 border border-amber-200 flex flex-col justify-between">
          ...
          <div className="flex items-baseline gap-1.5 my-1">
            <span className="text-2xl font-black text-amber-900">{stats.totalRemainingSks2026}</span>
            <span className="text-xs text-amber-800 font-bold">SKS</span>
          </div>
      ...
      {activeTab === 'conversion' && (
        <>
            ...
            <div className="flex flex-wrap items-center gap-1.5 bg-slate-100 p-1.5 rounded-2xl">
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all cursor-pointer ${
                  filterStatus === 'all' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-700 hover:text-slate-900'
                }`}
              >
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all cursor-pointer ${
                  filterStatus === 'converted' ? 'bg-emerald-600 text-white shadow-sm' : 'text-slate-700 hover:text-slate-900'
                }`}
              >
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center gap-1 ${
                  filterStatus === 'pilihan' ? 'bg-purple-600 text-white shadow-sm' : 'text-purple-900 bg-purple-100/70 hover:bg-purple-100'
                }`}
              >
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all cursor-pointer ${
                  filterStatus === 'multimatch' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-700 hover:text-slate-900'
                }`}
              >
          ...
          {/* Mobile Scroll Hint */}
          <div className="block lg:hidden mb-3 p-2.5 rounded-xl bg-blue-50 border border-blue-200 text-blue-900 text-xs font-bold flex items-center gap-2">
            <ArrowRightLeft className="w-4 h-4 shrink-0 text-blue-600" />
            <span>Geser horizontal untuk melihat hasil perbandingan Kurikulum 2022 vs 2026 secara lengkap ➔</span>
          </div>
            ...
            <div className="overflow-x-auto border border-slate-200 rounded-2xl custom-scrollbar">
              <table className="w-full text-left text-xs min-w-[720px]">
                <thead className="bg-slate-900 text-white font-bold uppercase tracking-wider">
                  <tr>
                    <th colSpan={4} className="py-3 px-4 bg-slate-900 text-blue-400 border-r-2 border-slate-700">
                      Kurikulum Lama (2022)
                    </th>
                  ...
                  <tr className="bg-slate-800 text-slate-300 text-[11px] border-t border-slate-700">
                    ...
                    <th className="py-2.5 px-3 text-center w-20 border-r-2 border-slate-700">Nilai</th>
                  ...
                  {filteredResults.map((item, idx) => {
                    ...
                    return (
                      <tr key={item.id} className={`${rowBgClass} transition-colors`}>
                        {/* 2022 Columns */}
                        <td className="py-3 px-3 font-mono font-bold text-slate-600">{item.kodeLama}</td>
                        <td className="py-3 px-4 font-bold text-slate-900">
                          <span>{item.namaLama}</span>
                          {isElective2022 && (
                            <span className="ml-2 px-2 py-0.5 rounded text-[9px] font-black bg-purple-100 text-purple-950 border border-purple-300 inline-block">
                              Pilihan 2022
                            </span>
                          )}
                        </td>
                        ...
                        <td className="py-3 px-3 text-center font-extrabold border-r-2 border-slate-200">
                          <span className="px-2.5 py-0.5 bg-blue-100 text-blue-950 rounded font-black text-xs">
                        ...
                        <td className="py-3 px-4 font-extrabold text-blue-950">
                          <div className="flex flex-wrap items-center gap-1.5">
                            ...
                            {isElective2026 && (
                              <span className="px-2 py-0.5 rounded-full text-[10px] font-black bg-purple-600 text-white shadow-xs inline-flex items-center gap-1">
                        ...
                        <td className="py-3 px-3 text-center font-extrabold">
                          {item.nilaiBaru ? (
                            <span className="px-2.5 py-0.5 bg-indigo-100 text-indigo-950 rounded font-black text-xs">
                        ...
                        <td className="py-3 px-4 text-center">
                          ...
                          {item.status === 'not_in_catalog' && (
                            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-black bg-amber-100 text-amber-950 border border-amber-300">
                              <HelpCircle className="w-3 h-3 text-amber-700" /> Luar Katalog
                            </span>
                          )}
                        </td>
                      </tr>
        ...
        <>
          <div className="mb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="relative w-full sm:w-72">
              ...
              <input
                type="text"
                aria-label="Cari mata kuliah 2026 yang belum diambil"
              ...
              <tbody className="divide-y divide-slate-100 font-medium">
                {filteredRemaining.map((course, idx) => (
                  <tr key={course.kode_baru} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="py-3 px-3 font-mono font-bold text-amber-900">{course.kode_baru}</td>
                    <td className="py-3 px-4 font-extrabold text-slate-900 flex items-center gap-2">
                      ...
                      {course.sem_baru.toLowerCase().includes('pilihan') && (
                        <span className="px-2 py-0.5 bg-purple-100 text-purple-950 rounded-full text-[10px] font-black border border-purple-200 inline-flex items-center gap-1">
                          <Sparkles className="w-2.5 h-2.5 text-purple-700" /> MK Pilihan
                        </span>
                      )}
                    </td>
                    <td className="py-3 px-3 text-center">
                      <span className="px-2.5 py-0.5 bg-amber-100 text-amber-950 rounded-full font-black text-xs">
                        {course.sks_baru} SKS
                      </span>
                    </td>
